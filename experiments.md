...
| Q7  | SELECT DISTINCT ?book ?author WHERE {<br>?book rdf:type dbpedia-owl:Book . (tp1)<br>?book dbpedia-owl:author ?author. (tp2)  }<br> LIMIT 100 |  BGP_1{  (tp1):   ?s1     rdf:type     dbpedia-owl:Book   (tp2):   ?s1   dbpedia-owl:author     ?o2   }   BGP_2{<br> (tp2'_a):   ?s1   dbpedia-owl:author   ?o1  <br>(tp2'_b):   ?s1    dbpedia-owl:author     ?o2  }  |  1 | 0,5 |
...
| Q30  | SELECT DISTINCT ?type ?label WHERE {<br>?s a ?type . (tp1)<br>OPTIONAL { ?type rdfs:label ?label . (tp2)}<br>   FILTER LANGMATCHES(LANG(?label), "EN")  }<br>LIMIT 100  |  BGP_1{  
<br> (tp1_equiv):  ?s1     rdf:type    ?o1 <br>(tp2):    ?s1    rdfs:label     ?o2 }  | 1 | 1 |

...
The first query (finalement il y a pas des cercle..).  <br><br>
The second query, seeks to find birth and death places, of people that lived in France. From a syntactical view of this query, there exist a cycle:  tp1 (?place) ---> tp2 (?place) ---> tp3 (?place) ---> tp3 (?person) ---> tp1 (?person). But during nested loop evaluation, the TPF client uses another join ordering strategy. First, it evaluates (tp2) and pushes "?place" mappings in object of both (tp1), (tp3). Then, "?someone" mappings of (tp1) are pushed into (tp3) and vice versa. In other words, the TPF client implements the joins: tp2 . tp1 (?place ---> ?place),  tp2 . tp3 (?place ---> ?place), tp1 . tp3 (?someone ---> ?someone) and finally  tp3 . tp1 (?someone ---> ?someone). Each join step is deduced by LIFT, resulting to different versions of (tp1), (tp3) as we see int the second deduced BGP of the Table above. Note, that LIFT deduces and aditional false self join tp3 . tp3 (?someone ---> ?someone), that can easily be pruned as pointed in the article.

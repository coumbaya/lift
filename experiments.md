...
| Q2           | SELECT ?title ?classification WHERE { ?author foaf:name "Anne De Paepe" . (tp1) \   publication dc:creator ?author . (tp2)  \   ?publication dc:title ?title . (tp3) \   ?publication ugent-biblio:classification  ?classification .   (tp4) } |     |      |         |
...
| Q6           | CONSTRUCT  { \   ?artist a dbpedia-owl:Artist.  ?artist dbpedia-owl:birthDate ?date.  }    WHERE { \   ?artist dbpedia-owl:influencedBy dbpedia:Pablo_Picasso . (tp1) \   ?artist a dbpedia-owl:Artist . (tp2) \   ?artist dbpedia-owl:birthDate ?date . (tp3)   }         |     |           |        |
...
| Q9           | SELECT DISTINCT ?artist ?band WHERE { \   {dbpedia:Queen_(band) dbpedia-owl:bandMember ?artist . (tp1) } \   UNION \   {  dbpedia:Queen_(band)   dbpedia-owl:formerBandMember ?artist . (tp2)  } \   ?artist dbpedia-owl:associatedBand ?band . (tp3) } |     |          |      |

...
| Q24 | SELECT ?clubName ?playerName WHERE { \   ?club a dbpedia-owl:SoccerClub . (tp1) \   ?club  dbpedia-owl:ground ?city . (tp2) \   ?club  rdfs:label ?clubName . (tp3) \   ?player dbpedia-owl:team ?club . (tp4) \   ?player dbpedia-owl:birthPlace ?city . (tp5) \   ?player rdfs:label ?playerName . (tp6) \   ?city dbpedia-owl:country dbpedia:Spain. (tp7) \      FILTER LANGMATCHES(LANG(?clubName), "EN") \      FILTER LANGMATCHES(LANG(?playerName), "EN") } |  BGP_1{  (tp7): ?s1     dbpedia-owl:country     dbpedia:Spain.   (tp2): ?s2     dbpedia-owl:ground     ?s1   (tp5): ?s3     dbpedia-owl:birthPlace  ?s1   (tp1): ?s2   rdf:type    dbpedia-owl:SoccerClub   (tp3): ?s2  rdf:label     ?o5                                (tp4'_a):  ?s6     dbpedia-owl:team    ?s2  (tp6):  ?s3     rdf:label     ?o7   (tp4): ?s3   dbpedia-owl:team   ?s2   (tp4'_b): ?s6     dbpedia-owl:team   ?s2   (tp5'_a): ?s3     dbpedia-owl:birthPlace  ?s2  }  BGP_2{  (tp5'_b):  ?s1     dbpedia-owl:birthPlace     ?o1  (tp7'_a):   ?s2    dbpedia-owl:country     INJECTEDsubj(tp5'_b)    (tp4'_c):  ?s     dbpedia-owl:team     INJECTEDsubj(tp7'_a)  (tp6'_a):   INJECTEDsubj(tp4'_c)    rdf:label     ?o  (tp2'_a):   ?s     dbpedia-owl:ground     INJECTEDsubj(tp7'_a)   (tp5'_b):  ?s     dbpedia-owl:birthPlace      INJECTEDsubj(tp7'_a)  (tp5'_c): INJECTEDsubj(tp4'_c) dbpedia-owl:birthPlace INJECTEDsubj(tp7'_a)  }  | 1 | 0,32 |
| Q25 | SELECT DISTINCT ?coauthorName { \   ?publication1 dc:creator ?coauthor . (tp1) \   ?coauthor foaf:name "Etienne Vermeersch". (tp2) \   ?coauthor foaf:name ?coauthorName . (tp3) \   XOR { \   ?publication2 dc:creator ?coauthor .  (tp4) \   ?coauthor   foaf:name "Freddy Mortier". (tp5) \   ?coauthor foaf:name ?coauthorName. (tp6) } } | BGP_1{  (tp2): Deduced LDF_2: ?s     foaf:name     "Etienne Vermeersch"   (tp1):   Deduced LDF_3: ?s     dc:creator    INJECTEDsubj(tp2)  (tp3):  Deduced LDF_4: INJECTEDsubj(tp2)     foaf:name     ?o   }   BGP_2{   (tp6): Deduced LDF_6: ?s     foaf:name     "Freddy Mortier"   (tp4): Deduced LDF_7: ?s     dc:creator     INJECTEDsubj(tp6)   (tp4'_a): Deduced LDF_8: INJECTEDsubj(tp4)    dc:creator     ?o    (tp4'_b): Deduced LDF_9: INJECTEDsubj(tp4)    dc:creator  INJECTEDsubj(LDF_6)   (tp4'_c):  Deduced LDF_10: ?s     dc:creator     INJECTEDobj(tp4'_a)  (tp5): Deduced LDF_11: INJECTEDobj(tp4)    foaf:name    ?o   (tp4'_d): Deduced LDF_12: INJECTEDsubj (tp4)    dc:creator     INJECTEDobj(tp4'_a)   }  | 1 | 0,44 |
| Q26 | SELECT distinct ?license  WHERE { \   ?ontology a owl:Ontology . (tp1) \   ?ontology  dc:license ?license. (tp2) } | BGP_1{   (tp2):    ?s     dc:license     ?o   (tp1_equiv):    ?injectsubj(tp2)     rdf:type    owl: Ontology  }   BGP_2{   (tp2):    ?s      rdf:type     ?o  (tp1'_a):    ?injectsubj(tp2)     rdf:type    owl: Ontology  }   | 1 | 0,5 |
| Q27 | SELECT ?entity ?label ?comment  WHERE { \   ?entity a dbpedia-owl:MythologicalFigure . (tp1) \   ?entity  rdfs:label ?label . (tp2) \   ?entity dc:subject  dbpedia-cat:Women_in_Greek_mythology . (tp3) \   ?entity rdfs:comment ?comment . (tp4) \      FILTER(langMatches(lang(?label), "EN")) \      FILTER(langMatches(lang(?comment), "EN")) }  |  BGP_1{  (tp3):    ?s   dc:subject    dbpedia-cat::Women_in_Greek_mythology  (tp1_a):    ?injectsubj(tp3)  rdf:type   dbpedia-owl:MythologicalFigure  (tp4):     ?injectsubj(tp3)      rdfs:comment     ?o  (tp2):     ?injectsubj(tp3)       rdfs:label      ?o   }   BGP_2{   (tp3'_a):    ?s   dc:subject    dbpedia-owl:MythologicalFigure   (tp1'_a):    ?injectsubj(tp3'_a)    dc:subject  dbpedia-owl:MythologicalFigure  (tp4'_a):     ?injectsubj(tp3'_a)      rdfs:comment     ?o  (tp2'_a):     ?injectsubj(tp3'_a)       rdfs:label      ?o }   | 1 | 0,5 |
...
| Q29  | SELECT DISTINCT ?subject ?label WHERE { \   { ?subject dc:title ?label  (tp1) } \   UNION \   { ?subject rdfs:label ?label  (tp2) } } \   LIMIT 100  |  BGP_1{  (tp1):  ?s     dc:itle     ?o }  BGP_2{  (tp2):   ?s    rdfs:label        ?o  }   | 0/0 | 0/0 |
| Q30  | SELECT DISTINCT ?type ?label WHERE { \   ?s a ?type . (tp1) \   OPTIONAL { ?type rdfs:label ?label . (tp2)} \      FILTER LANGMATCHES(LANG(?label), "EN")  } \   LIMIT 100  |  BGP_1{   (tp1_equiv):  ?s1     rdf:type    ?o1 (tp2):    ?s1    rdfs:label     ?o2 }  | 1 | 1 |

...
| Q30  | SELECT DISTINCT ?type ?label WHERE { \   ?s a ?type . (tp1) \   OPTIONAL { ?type rdfs:label ?label . (tp2)} \      FILTER LANGMATCHES(LANG(?label), "EN")  } \   LIMIT 100  |  BGP_1{   (tp1_equiv):  ?s1     rdf:type    ?o1 (tp2):    ?s1    rdfs:label     ?o2 } 
| 1 | 1 |
